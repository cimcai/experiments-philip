...
It's primarily a tool for **tracking and visualizing individual ant paths**. The current tracks are useful but far from perfect — identity switching still happens substantially more than it should, especially inside dense cluster events on the load. Improving track quality is mostly hyperparameter tuning (`MAX_LOST`, cluster-event detection thresholds, continuation-edge generosity, count-conservation tolerances) plus better collision handling: re-matching ants at cluster exits using appearance features, motion priors, or longer-window re-id rather than the current count-conserved 1:1 / 2:2 bridges.

Even with the current tracks — and especially as they're improved — the labeled paths can be used to:

- **Build stronger intuition about individual ant movement and behavior** — scrub to a moment, focus on one ant, watch what local rule its motion looks consistent with.
- **Cluster roles, behaviors, and contributions** across ants — which individuals do most of the pushing, which are scouts, which are recruiters, which are hangers-on.
- **Rate the fidelity of simulations** against the real ant movement, and ultimately learn or guide virtual-ant policies rewarded to solve the task the way the real colony does. That's the lever for separating colony-level outcomes from individual-level representations and intelligence, and what makes this a candidate model system for studying emergent collective behavior more broadly.
